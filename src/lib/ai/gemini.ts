  ...
  async generateStream(
        ...
        try {
          ...
          streamResult = await model.generateContentStream({
            contents: [
              ...
              {
                role: "user" as const,
                parts: [{ text: lastUserMsg?.parts[0].text || "" }],
              },
            ],
          });
          break;
          ...
          if (isRateLimit && attempt < 2) {
            ...
            console.error(
              `[Gemini] Model ${modelName} rate limited, retrying in ${delay}ms (attempt ${attempt + 1}/3):`,
              err,
            );
    ...
    if (!fullContent) {
      if (lastError) {
        ...
        fullContent =
          "The AI service is currently unavailable. Check your GEMINI_API_KEY or try again later.";
      } else {
