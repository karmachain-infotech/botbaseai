...
function AdminUsers() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (ids: string[]) =>
      Promise.all(ids.map((id) => adminDeleteUser({ data: { userId: id } }))),
    onSuccess: () => {
  ...
  const toggleAdminMutation = useMutation({
    mutationFn: ({
      ...
      userId: string;
      makeAdmin: boolean;
    }) => adminUpdateUser({ data: { userId, is_admin: makeAdmin } }),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ["admin-users"] }),
  });
          ...
          {selected.size > 0 && (
            <>
              <Button
                ...
                size="sm"
                onClick={() => {
                  if (confirm(`Delete ${selected.size} user(s)?`))
              ...
              <thead>
                <tr className="border-b border-zinc-800 bg-zinc-950">
                  ...
                  <SortHeader
                    ...
                    onClick={toggleSort}
                  />
                  <th className="p-4 text-left text-zinc-400 font-medium">
                    Name
                  </th>
                  <SortHeader
                    label="Plan"
              ...
              <tbody>
                ...
                {data?.users?.map((u: AdminUser) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${u.is_admin ? "bg-purple-900/50 text-purple-300" : "bg-zinc-800 text-zinc-500"}`}
                      >
...
function SortHeader({
  ...
  label: string;
  sortKey: SortKey;
  current: { key: SortKey; dir: string };
...
function PlanBadge({ plan }: { plan?: string }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-xs font-medium ${colors[plan || "free"] ?? colors.free}`}
    >
      {plan || "free"}
    </span>
  );
  ...
  return (
    <div className="space-y-3">
      {Array.from({ length: 8 }).map((_, i) => (
        <Skeleton key={i} className="h-12 rounded-xl bg-zinc-800" />
      ))}
    </div>
  );
}
