...
import { adminGetSettings, adminUpdateSetting } from "@/lib/server-functions/admin";
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-zinc-400">Configure global platform settings and feature flags.</p>
      </div>
      ...
      <div className="max-w-2xl space-y-6">
        ...
        <Section icon={Shield} title="Maintenance Mode">
          <SettingRow label="Enable Maintenance Mode">
            <Toggle enabled={!!localSettings.maintenance_mode} onClick={() => toggleAndSave("maintenance_mode")} color="red" />
          </SettingRow>
        </Section>
...
function Toggle({ enabled, onClick, color = "green" }: { enabled: boolean; onClick: () => void; color?: string }) {
  const colors: Record<string, string> = { green: "bg-green-600", red: "bg-red-600" };
  return (
  ...
  return (
    <div className="p-6">
      ...
      <div className="mt-6 space-y-6">
        {Array.from({ length: 4 }).map((_, i) => <Skeleton key={i} className="h-48 rounded-xl bg-zinc-800" />)}
      </div>
    </div>
  );
}
