...
function AdminBilling() {
  ...
  function exportCSV(type: "subscriptions" | "transactions") {
    if (!data) return;
    const items = (type === "subscriptions"
      ? data.subscriptions
    ...
    const rows = items.slice(0, 500).map((item) =>
      headers
        .map((h) => {
          const v =
            item[h.toLowerCase().replace(/\s/g, "_")] ??
            item[h.toLowerCase()] ??
            "";
  ...
  if (error)
    return (
      <div className="p-6 text-red-400">
        Failed to load billing data. Stripe may not be configured.
      </div>
    );
  if (!data) return null;
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-800 bg-zinc-950">
                {["User", "Status", "Amount", "Next Billing", "Sub ID"].map(
                  (h) => (
                    <th
                      key={h}
                      className="px-4 py-3 text-left text-xs font-medium uppercase text-zinc-400"
                    >
                      {h}
                    </th>
                  ),
...
function SubStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${colors[status] || "bg-zinc-800 text-zinc-400"}`}
    >
      {status}
    </span>
  );
...
function TxStatusBadge({ status }: { status: string }) {
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-xs font-medium ${status === "paid" ? "bg-green-900/50 text-green-400" : "bg-zinc-800 text-zinc-400"}`}
    >
      {status}
    </span>
  );
