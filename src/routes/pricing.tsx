...
export const Route = createFileRoute("/pricing")({
  head: () => ({
    meta: [
      ...
      { description: "Predictable pricing and scalable plans for AI support agents." },
    ],
  }),
  component: Pricing,
});
...
const addOns = [
  { title: "Auto recharge credits", price: "$40", unit: "per 1,000 message credits" },
...
const comparison = [
  {
    section: "Usage",
    rows: [
      { label: "Message credits / mo", values: ["50", "500", "4,000", "15,000", "Custom"] },
      { label: "AI agents", values: ["1", "1", "1", "1", "Custom"] },
      { label: "AI Actions / agent", values: ["0", "5", "8", "12", "Custom"] },
      { label: "Training content", values: ["400 KB", "10 MB", "20 MB", "40 MB", "Custom"] },
  ...
  },
  {
    ...
    rows: [
      ...
      { label: "Help desk, Voice, Telephony", values: ["—", "—", "✓", "✓", "✓"] },
...
const faqs = [
  { q: "What is a message credit?", a: "Each reply your AI agent sends to a customer consumes one message credit. Credits reset at the start of every billing cycle." },
  { q: "Do credits roll over?", a: "Unused message credits do not roll over to the next month. You can enable auto recharge to never run out." },
  { q: "When do my credits renew?", a: "Credits renew on your billing renewal date — monthly or yearly depending on your plan." },
  { q: "Can I add more team members?", a: "Yes. Each plan includes a number of members, and you can add extra agents and seats with add-ons." },
  { q: "How much data can I train on?", a: "Training content limits vary by plan, from 400 KB on Free up to 40 MB on Pro, with custom limits on Enterprise." },
  { q: "Can I change plans anytime?", a: "Absolutely. You can upgrade, downgrade or cancel at any time from your billing settings." },
];
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-4 pt-20 pb-10 text-center sm:px-6 lg:px-8">
          <h1 className="text-balance text-4xl font-extrabold tracking-tight sm:text-5xl">
            Predictable pricing, <span className="text-gradient">scalable plans</span>
          </h1>
          <p className="mx-auto mt-5 max-w-xl text-lg text-muted-foreground">
            Start free and upgrade as your support grows. Save 20% with yearly billing.
          </p>
          ...
          <div className="mt-8 inline-flex items-center gap-3 rounded-full border border-border bg-card p-1">
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-sm font-medium transition-colors ${
                !yearly ? "bg-gradient-brand text-primary-foreground" : "text-muted-foreground"
              }`}
            >
              Monthly
            </button>
              ...
              <div className="mt-4">
                {p.monthly === null ? (
                  ...
                  <>
                    ...
                    {yearly && p.yearly ? (
                      <p className="mt-1 text-xs text-muted-foreground">billed ${p.yearly.toLocaleString()}/yr</p>
                    ) : null}
                  </>
                )}
              </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <h2 className="mb-8 text-center text-2xl font-bold tracking-tight">Add-ons</h2>
        <div className="grid gap-6 md:grid-cols-3">
          {addOns.map((a) => (
            <div key={a.title} className="rounded-2xl border border-border bg-card p-6">
              <h3 className="font-semibold">{a.title}</h3>
              <p className="mt-3 text-2xl font-extrabold text-gradient">{a.price}</p>
          ...
          <table className="w-full min-w-[760px] text-sm">
            <thead>
              <tr className="border-b border-border bg-card">
                ...
                {plans.map((p) => (
                  <th key={p.name} className="p-4 text-center font-semibold">{p.name}</th>
                ))}
              </tr>
            </thead>
            <tbody>
          ...
          {faqs.map((f, i) => (
            <div key={f.q} className="rounded-xl border border-border bg-card">
              ...
              >
                ...
                <ChevronDown className={`h-5 w-5 flex-shrink-0 text-muted-foreground transition-transform ${openFaq === i ? "rotate-180" : ""}`} />
              </button>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-secondary to-card p-12 text-center shadow-card">
          ...
          <div className="relative">
            <h2 className="text-3xl font-bold tracking-tight">Make customer service your competitive edge</h2>
            <Link to="/signup" className="mt-8 inline-flex items-center gap-2 rounded-lg bg-gradient-brand px-6 py-3 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90">
