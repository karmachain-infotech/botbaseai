    ...
    <div className="p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        <StatCard
          ...
          change={`${data.userGrowthPercent >= 0 ? "+" : ""}${data.userGrowthPercent}%`}
          trend={data.userGrowthPercent >= 0 ? "up" : "down"}
          icon={Users}
        />
        <StatCard
          title="Chatbots"
          value={data.totalChatbots.toLocaleString()}
          icon={Bot}
        />
        <StatCard
          title="Messages"
        ...
        <StatCard
          ...
          value={data.churnedUsersThisMonth.toString()}
          icon={UserMinus}
          negative
        />
      </div>
        ...
        <ChartCard title="MRR / ARR">
          <div className="flex h-[300px] flex-col items-center justify-center gap-4">
            ...
            <div className="text-center">
              ...
              <p className="text-3xl font-bold text-white">
                ${(data.monthlyRevenue * 12).toLocaleString()}
              </p>
            </div>
          </div>
        </ChartCard>
        ...
        <ActivityFeed
          ...
          items={[
            ...data.recentSignups.map((s) => ({
              label: `New signup: ${s.name || s.email}`,
              sub: s.email,
              time: s.created_at,
            })),
            ...data.recentChatbots.map((c) => ({
              label: `New chatbot: ${c.name}`,
              sub: c.user_email,
              time: c.created_at,
            })),
            ...data.recentSubscriptionChanges.map((sc) => ({
              label: `Subscription ${sc.action}: ${sc.user_email}`,
              sub: `Plan: ${sc.plan}`,
              time: sc.created_at,
            })),
          ]}
...
function StatCard({
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-5">
      ...
      {change && (
        <div
          className={`mt-1 flex items-center gap-1 text-xs ${negative ? "text-red-400" : trend === "up" ? "text-green-400" : "text-red-400"}`}
        >
          {trend === "up" ? (
    ...
    <div className="p-6">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        {Array.from({ length: 4 }).map((_, i) => (
          <Skeleton key={i} className="h-80 rounded-xl bg-zinc-800" />
        ))}
      </div>
    </div>
  );
