...
import { ArrowRight, BookOpen, GraduationCap, HelpCircle, Users, Radio, FileText, Search, Sparkles } from "lucide-react";
...
export const Route = createFileRoute("/resources")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Guides, tutorials, docs and community resources for building better AI support agents." },
    ],
  }),
  component: Resources,
});
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-5xl px-4 pt-24 pb-16 text-center sm:px-6 lg:px-8">
          <h1 className="text-balance text-4xl font-extrabold tracking-tight sm:text-6xl">
            Resources to help you <span className="text-gradient">build better agents</span>
          </h1>
          <p className="mx-auto mt-6 max-w-2xl text-lg text-muted-foreground">
            Documentation, guides, tutorials, and community resources — everything you need to succeed with BotbaseAI.
          </p>
          <div className="mx-auto mt-8 flex max-w-lg items-center gap-2 rounded-xl border border-border bg-card p-2">
            ...
            <input
              ...
              readOnly
            />
            <span className="rounded-lg bg-gradient-brand px-3 py-1.5 text-xs font-medium text-primary-foreground">Search</span>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="mb-12 text-center">
          <p className="text-sm font-semibold uppercase tracking-wider text-primary">Popular guides</p>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {popularGuides.map((g) => (
            <div key={g.title} className="group cursor-pointer rounded-2xl border border-border bg-card p-6 transition-colors hover:border-primary/50">
              <div className="flex items-center gap-2">
                <Sparkles className="h-4 w-4 text-primary" />
                <span className="text-xs text-muted-foreground">{g.readTime}</span>
              </div>
              <h3 className="mt-3 font-semibold group-hover:text-primary transition-colors">{g.title}</h3>
      ...
      <section className="border-y border-border/60 bg-card/40">
        <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
          <div className="mb-12 text-center">
            <p className="text-sm font-semibold uppercase tracking-wider text-primary">All resources</p>
            <h2 className="mx-auto mt-3 max-w-2xl text-balance text-3xl font-bold tracking-tight sm:text-4xl">
              Browse by category
            </h2>
          </div>
            ...
            {categories.map((cat) => (
              <div key={cat.title} className="rounded-2xl border border-border bg-card p-6">
                ...
                <ul className="mt-4 space-y-2">
                  {cat.articles.map((a) => (
                    <li key={a} className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors cursor-pointer">
        ...
        <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-secondary to-card p-12 text-center shadow-card">
          ...
          <div className="relative">
            ...
            <p className="mx-auto mt-4 max-w-xl text-muted-foreground">
              Our support team is here to help you get the most out of BotbaseAI.
            </p>
