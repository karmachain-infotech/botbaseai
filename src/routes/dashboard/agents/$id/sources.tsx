...
const sourceTypeConfig: Record<SourceType, { label: string; icon: typeof FileText }> = {
...
function AgentSources() {
  ...
  async function handleAddSource(type: SourceType) {
    ...
    try {
      const opts: { chatbotId: string; type: SourceType; name: string; content?: string; fileBase64?: string } = {
        chatbotId: id,
        type,
        name: "",
      };
  ...
  const statusIcon = (status: string) => {
    switch (status) {
      case "trained": return <CheckCircle className="h-4 w-4 text-primary" />;
      case "failed": return <XCircle className="h-4 w-4 text-destructive" />;
      case "processing": return <RefreshCw className="h-4 w-4 animate-spin text-muted-foreground" />;
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-4">
        {(Object.entries(sourceTypeConfig) as [SourceType, { label: string; icon: typeof FileText }][]).map(([type, config]) => (
          <button key={type} onClick={() => setShowAdd(showAdd === type ? null : type)}
            className={`flex items-center gap-3 rounded-xl border p-4 text-left transition-colors ${
              showAdd === type ? "border-primary bg-primary/5" : "border-border bg-card hover:border-primary/50"
            }`}>
        ...
        <div className="mt-4 rounded-xl border border-border bg-card p-4">
          {showAdd === "url" && (
            <div className="flex gap-3">
              ...
              <button onClick={() => handleAddSource("url")} disabled={!urlInput || adding}
          ...
          {showAdd === "text" && (
            <div className="space-y-3">
              ...
              <button onClick={() => handleAddSource("text")} disabled={!textInput || adding}
                className="rounded-lg bg-gradient-brand px-4 py-2.5 text-sm font-semibold text-primary-foreground disabled:opacity-60">
          ...
          {showAdd === "qa" && (
            <div className="space-y-3">
              ...
              <button onClick={() => handleAddSource("qa")} disabled={!textInput || adding}
                className="rounded-lg bg-gradient-brand px-4 py-2.5 text-sm font-semibold text-primary-foreground disabled:opacity-60">
          ...
          {showAdd === "file" && (
            <div className="space-y-3">
              <div onClick={() => fileInputRef.current?.click()}
                ...
                <p className="text-sm font-medium">{fileInput ? fileInput.name : "Click to select a file"}</p>
                <p className="text-xs text-muted-foreground">PDF, DOCX, TXT</p>
              </div>
              <input ref={fileInputRef} type="file" accept=".pdf,.docx,.txt" className="hidden"
                onChange={(e) => setFileInput(e.target.files?.[0] ?? null)} />
              <button onClick={() => handleAddSource("file")} disabled={!fileInput || adding}
                className="rounded-lg bg-gradient-brand px-4 py-2.5 text-sm font-semibold text-primary-foreground disabled:opacity-60">
