...
import {
  ...
  updateChatbot,
  deleteChatbot,
} from "@/lib/server-functions/chatbots";
...
function AgentSettings() {
  ...
  useEffect(() => {
    ...
    getAppUrl()
      .then((url) => {
        if (mountedRef.current) setEmbedBase(url);
      })
      .catch(() => {});
    async function load() {
      try {
        ...
        if (bot && mountedRef.current) {
          ...
          setPrimaryColor(
            (data.widget_config as { primaryColor?: string })?.primaryColor ??
              "#7c3aed",
          );
          setBackgroundColor(
            (data.widget_config as { backgroundColor?: string })
              ?.backgroundColor ?? "#ffffff",
          );
          setTextColor(
            (data.widget_config as { textColor?: string })?.textColor ??
              "#1f2937",
          );
          setGreeting(
            (data.widget_config as { greeting?: string })?.greeting ?? "",
          );
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Add this script to your website to embed the chat widget. Place
            ...
            <div className="rounded-xl border border-border bg-background p-4">
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(
                      `<script src="${embedBase}/widget.js" data-bot-id="${id}" data-base-url="${embedBase}"></script>`,
                    );
                  }}
              ...
              <pre className="mt-2 overflow-x-auto text-xs font-mono">{`<script src="${embedBase}/widget.js" data-bot-id="${id}" data-base-url="${embedBase}"></script>`}</pre>
            </div>
            <div className="rounded-xl border border-border bg-background p-4">
              <p className="text-xs font-medium text-muted-foreground">
                Agent ID
            ...
            <div className="rounded-xl border border-dashed border-border bg-background p-4">
              <p className="text-sm text-muted-foreground">
                Before the widget appears on your site, publish this agent by
                changing its status to <strong>Live</strong> in the General tab.
                The widget will use the primary color and greeting configured in
                the Appearance tab.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="mt-1.5 text-xs text-muted-foreground">
                Only these domains will be allowed to embed your chat widget.
              </p>
            </div>
          </div>
        )}
            ...
            <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    This will permanently delete the agent and all its training
                    data.
                  </p>
                </div>
              </div>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      ...
                      and all of its training data, conversation history, and
