...
import {
  listConversations,
  getConversation,
} from "@/lib/server-functions/conversations";
  ...
  async function openConversation(convId: string) {
    try {
      ...
      if (mountedRef.current) {
        ...
        const msgs =
          (conv as unknown as { messages: MsgRecord[] }).messages ?? [];
        setMessages(msgs);
      }
    } catch (err) {
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium truncate">
                      {conv.user_identifier ??
                        `Session: ${(conv.session_id ?? conv.id).slice(0, 12)}...`}
                    </p>
                  </div>
