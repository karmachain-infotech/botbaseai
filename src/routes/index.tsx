    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <MarqueeBand className="mt-6" baseSpeed={15}>
            {["OpenAI", "Sage", "Chuck E Cheese", "Synergym", "Postman", "Siemens"].map((logo) => (
              <span key={logo} className="text-lg font-bold tracking-tight text-muted-foreground opacity-70">
                {logo}
              </span>
            ))}
          </MarqueeBand>
      ...
      <Section>
        <StaggerContainer className="grid gap-6 md:grid-cols-3">
          {highlights.map((h) => (
            <motion.div key={h.title} variants={staggerItem} className="rounded-2xl border border-border bg-card p-8 shadow-card">
      ...
      <Section>
        ...
        <div className="grid gap-10 lg:grid-cols-2">
          <StaggerContainer className="space-y-4" staggerDelay={0.06}>
            {steps.map((s) => (
              <motion.div key={s.n} variants={staggerItem} className="flex gap-4 rounded-xl border border-border bg-card p-5">
      ...
      <Section>
        ...
        <StaggerContainer className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f) => (
            <motion.div key={f.title} variants={staggerItem} className="rounded-2xl border border-border bg-card p-6 transition-all duration-300 hover:border-primary/50 hover:shadow-card hover:-translate-y-1">
      ...
      <Section>
        ...
        <StaggerContainer className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {benefits.map((b) => (
            <motion.div key={b.title} variants={staggerItem} className="rounded-2xl border border-border bg-card p-6">
      ...
      <Section>
        ...
        <StaggerContainer className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {enterprise.map((e) => (
            <motion.div key={e.title} variants={staggerItem} className="rounded-2xl border border-border bg-card p-6">
      ...
      <Section>
        ...
        <StaggerContainer className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((t) => (
            <motion.div key={t.company} variants={staggerItem} className="rounded-2xl border border-border bg-card p-6 transition-all duration-300 hover:scale-[1.02] hover:shadow-card">
      ...
      <Section>
        ...
        >
          ...
          <StaggerContainer className="mt-10 grid gap-6 md:grid-cols-3">
            {security.map((s) => (
              <motion.div key={s.title} variants={staggerItem} className="rounded-xl border border-border bg-secondary/50 p-6">
...
function Heading({ eyebrow, title }: { eyebrow: string; title: string }) {
  return (
    ...
    >
      ...
      <TextReveal as="h2" className="mx-auto mt-3 max-w-2xl text-balance text-3xl font-bold tracking-tight sm:text-4xl">
        {title}
      </TextReveal>
    </motion.div>
  );
