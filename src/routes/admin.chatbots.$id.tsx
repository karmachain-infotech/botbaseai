...
function AdminChatbotDetail() {
  ...
  if (error)
    return <div className="p-6 text-red-400">Failed to load chatbot</div>;
  if (!data) return null;
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center gap-4">
        ...
        <span
          className={`rounded-full px-3 py-1 text-xs font-medium ${chatbot.status === "live" ? "bg-green-900/50 text-green-400" : "bg-zinc-800 text-zinc-400"}`}
        >
          {chatbot.status}
        </span>
      </div>
        ...
        <div className="space-y-6 lg:col-span-1">
          <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-6">
            ...
            <div className="space-y-3 text-sm">
              <div>
                ...
                <p className="font-mono text-xs text-zinc-400">{chatbot.id}</p>
              </div>
              <div>
                <p className="text-xs text-zinc-500">Language</p>
                <p className="text-white">{chatbot.language}</p>
              </div>
              <div>
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-6">
            ...
            <Link
              to="/admin/users/$id"
              params={{ id: owner.id }}
              className="flex items-center gap-3 rounded-lg border border-zinc-800 p-3 hover:bg-zinc-900"
            >
          ...
          <button
            onClick={() => {
              if (confirm(`Delete chatbot "${chatbot.name}"?`))
        ...
        <div className="space-y-6 lg:col-span-2">
          <div className="grid grid-cols-3 gap-4">
            <StatCard
              icon={MessageSquare}
              label="Messages"
              value={chatbot.message_count?.toLocaleString() || 0}
            />
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-6">
            ...
            <p className="whitespace-pre-wrap text-sm text-zinc-400">
              {chatbot.instructions || "No instructions set"}
            </p>
          </div>
