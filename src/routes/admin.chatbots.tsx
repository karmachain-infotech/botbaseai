...
function AdminChatbots() {
  ...
  const isDetail =
    pathname !== "/admin/chatbots" && pathname.startsWith("/admin/chatbots");
  ...
  const { data, isLoading, error } = useQuery<{
    ...
    queryFn: () =>
      adminListAllChatbots({ data: { search, status: statusFilter, page } }),
    placeholderData: keepPreviousData,
  });

  const deleteMutation = useMutation({
    mutationFn: (chatbotId: string) =>
      adminDeleteChatbot({ data: { chatbotId } }),
    onSuccess: () => {
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-zinc-400">
          All chatbots across the platform.
        </p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-red-400">Failed to load chatbots</div>
      ) : (
        <div className="overflow-x-auto rounded-xl border border-zinc-800">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-800 bg-zinc-950">
                {["Name", "Owner", "Status", "Messages", "Created", ""].map(
                  (h) => (
                    <th
                      key={h}
            ...
            <tbody className="divide-y divide-zinc-800">
              ...
              {data?.chatbots.map((bot: AdminChatbot) => (
                <tr
                  key={bot.id}
                  className="bg-zinc-950/50 hover:bg-zinc-900/50"
                >
                  ...
                  <td className="px-4 py-3 text-zinc-300">
                    {bot.message_count?.toLocaleString() || 0}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button
                        onClick={() => {
                          if (confirm(`Delete chatbot "${bot.name}"?`))
      ...
      {pages.map((p, i) =>
        p === "..." ? (
          ...
          <button
            key={p}
            onClick={() => {
              if (typeof p === "number") onPage(p);
            }}
            className={`min-w-[32px] rounded-lg px-2.5 py-1.5 text-xs font-medium transition-colors ${p === page ? "bg-red-600 text-white" : "text-zinc-400 hover:bg-zinc-800 hover:text-white"}`}
          >
            {p}
          </button>
        ),
